...
class Recon:
    ...
    def _crawl(self):
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            ...
            # Use a true loop condition that checks for either queued items OR active tasks
            while True:
                # 1. Feed the pool up to capacity limits
                while len(futures) < self.threads:
                    try:
                        # Use non-blocking get to avoid freezing if queue goes empty mid-check
                    ...
                    if abs_url and same_domain(url, abs_url):
                        with self.crawl_lock:
                            if abs_url not in visited:
                                visited.add(abs_url)
                                depth_map[abs_url] = depth + 1  # Calculated and locked safely
    ...
    def _enumerate_subdomains(self):
        ...
        with ThreadPoolExecutor(max_workers=self.threads) as executor:
            ...
            # Subdomains are a static pool list, so standard as_completed works perfectly here
